...
def compare_plot(x_base, y_base, x_compare1, y_compare1, x_compare2, y_compare2, xlabel, ylabel, title, filename, x_min, x_max, y_min, y_max, xticks,
                 yticks, label1, label2):
    fig, ax = plt.subplots()
    ax.plot(x_base, y_base, 'orange', label='Baseline Model')
    ax.plot(x_compare2, y_compare2, 'steelblue', label=label2)
    ax.plot(x_compare1, y_compare1, 'forestgreen', label=label1)
    ax.xaxis.set_ticks(np.arange(6, 16, xticks))
...
x_compare1, y_compare1 = get_data('fid50k', 22)
x_compare2, y_compare2 = get_data('fid50k_no_noise', 15)
compare_plot(x_base, y_base, x_compare1, y_compare1, x_compare2, y_compare2, 'Million Images Seen by the Discriminator', 'Fréchet Inception Distance',
             'Fréchet Inception Distance over the Training', 'fid_noise_all', 8, 15.5, 6, 9, 1, 1, 'New Model Without Noise', 'Baseline Model Without Noise')

# x_base, y_base = get_data('fid50k', 15)
# x_compare, y_compare = get_data('fid50k_no_noise', 15)
# compare_plot(x_base, y_base, x_compare, y_compare, 'Million Images', 'FID',
#              'Fréchet Inception Distance', 'fid_validation', 9, 16, 6, 7.5, 3, 1, 'baseline model without noise')
