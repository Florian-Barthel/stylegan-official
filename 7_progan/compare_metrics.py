...
def compare_plot(x_base, y_base, x_compare, y_compare, xlabel, ylabel, title, filename, x_min, x_max, y_min, y_max, xticks,
                 yticks, logarithmic=False):
    fig, ax = plt.subplots()
    ax.plot(x_base, y_base, 'orange', label='Baseline Model')
    ax.axvline(full_res_threshold, 0, 1, label='Full Resolution', c='orange', ls='--')
    ax.plot(x_compare, y_compare, 'green', label='Model Without Progressive Growing')
...
x_compare, y_compare = get_data('fid50k', 1)
compare_plot(x_base, y_base, x_compare, y_compare, 'Million Images seen by the Discriminator', 'Fréchet Inception Distance',
             'Fréchet Inception Distance over the Training', 'fid-remove-progan', 0, 15.5, 6, 20, 3, 2)
